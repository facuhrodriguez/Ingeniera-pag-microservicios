...
describe('Telefono e2e test', () => {
  const telefonoPageUrl = '/cliente/telefono';
  const telefonoPageUrlPattern = new RegExp('/cliente/telefono(\\?.*)?$');
  const username = Cypress.env('E2E_USERNAME') ?? 'user';
  const password = Cypress.env('E2E_PASSWORD') ?? 'user';
  const telefonoSample = {};
  ...
  beforeEach(() => {
    cy.intercept('GET', '/services/cliente/api/telefonos+(?*|)').as('entitiesRequest');
    cy.intercept('POST', '/services/cliente/api/telefonos').as('postEntityRequest');
    cy.intercept('DELETE', '/services/cliente/api/telefonos/*').as('deleteEntityRequest');
  });
  ...
  afterEach(() => {
    if (telefono) {
      cy.authenticatedRequest({
        method: 'DELETE',
        url: `/services/cliente/api/telefonos/${telefono.id}`,
      }).then(() => {
        telefono = undefined;
      });
    }
  ...
  it('Telefonos menu should load Telefonos page', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem('cliente/telefono');
  ...
  describe('Telefono page', () => {
    describe('create button click', () => {
      ...
      it('should load create Telefono page', () => {
        ...
        cy.url().should('match', new RegExp('/cliente/telefono/new$'));
    ...
    describe('with existing value', () => {
      beforeEach(() => {
        cy.authenticatedRequest({
          method: 'POST',
          url: '/services/cliente/api/telefonos',
          body: telefonoSample,
        }).then(({ body }) => {
          telefono = body;
      ...
      it('last delete button click should delete instance of Telefono', () => {
        cy.intercept('GET', '/services/cliente/api/telefonos/*').as('dialogDeleteRequest');

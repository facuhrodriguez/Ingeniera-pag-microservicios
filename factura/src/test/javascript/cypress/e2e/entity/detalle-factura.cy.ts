...
describe('DetalleFactura e2e test', () => {
  const detalleFacturaPageUrl = '/factura/detalle-factura';
  ...
  beforeEach(() => {
    cy.intercept('GET', '/services/factura/api/detalle-facturas+(?*|)').as('entitiesRequest');
    cy.intercept('POST', '/services/factura/api/detalle-facturas').as('postEntityRequest');
    cy.intercept('DELETE', '/services/factura/api/detalle-facturas/*').as('deleteEntityRequest');
  });
  ...
  afterEach(() => {
    if (detalleFactura) {
      cy.authenticatedRequest({
        method: 'DELETE',
        url: `/services/factura/api/detalle-facturas/${detalleFactura.id}`,
      }).then(() => {
  ...
  it('DetalleFacturas menu should load DetalleFacturas page', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem('factura/detalle-factura');
  ...
  describe('DetalleFactura page', () => {
    describe('create button click', () => {
      ...
      it('should load create DetalleFactura page', () => {
        ...
        cy.url().should('match', new RegExp('/factura/detalle-factura/new$'));
    ...
    describe('with existing value', () => {
      beforeEach(() => {
        cy.authenticatedRequest({
          method: 'POST',
          url: '/services/factura/api/detalle-facturas',
      ...
      it('last delete button click should delete instance of DetalleFactura', () => {
        cy.intercept('GET', '/services/factura/api/detalle-facturas/*').as('dialogDeleteRequest');

...
describe('Factura e2e test', () => {
  const facturaPageUrl = '/factura/factura';
  ...
  beforeEach(() => {
    cy.intercept('GET', '/services/factura/api/facturas+(?*|)').as('entitiesRequest');
    cy.intercept('POST', '/services/factura/api/facturas').as('postEntityRequest');
    cy.intercept('DELETE', '/services/factura/api/facturas/*').as('deleteEntityRequest');
  });
  ...
  afterEach(() => {
    if (factura) {
      cy.authenticatedRequest({
        method: 'DELETE',
        url: `/services/factura/api/facturas/${factura.id}`,
      }).then(() => {
        factura = undefined;
      });
    }
  ...
  it('Facturas menu should load Facturas page', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem('factura/factura');
  ...
  describe('Factura page', () => {
    describe('create button click', () => {
      ...
      it('should load create Factura page', () => {
        ...
        cy.url().should('match', new RegExp('/factura/factura/new$'));
    ...
    describe('with existing value', () => {
      beforeEach(() => {
        cy.authenticatedRequest({
          method: 'POST',
          url: '/services/factura/api/facturas',
          body: facturaSample,
        }).then(({ body }) => {
          factura = body;
      ...
      it('last delete button click should delete instance of Factura', () => {
        cy.intercept('GET', '/services/factura/api/facturas/*').as('dialogDeleteRequest');

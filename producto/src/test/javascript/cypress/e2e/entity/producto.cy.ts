...
describe('Producto e2e test', () => {
  const productoPageUrl = '/producto/producto';
  const productoPageUrlPattern = new RegExp('/producto/producto(\\?.*)?$');
  ...
  beforeEach(() => {
    cy.intercept('GET', '/services/producto/api/productos+(?*|)').as('entitiesRequest');
    cy.intercept('POST', '/services/producto/api/productos').as('postEntityRequest');
    cy.intercept('DELETE', '/services/producto/api/productos/*').as('deleteEntityRequest');
  });
  ...
  afterEach(() => {
    if (producto) {
      cy.authenticatedRequest({
        method: 'DELETE',
        url: `/services/producto/api/productos/${producto.id}`,
      }).then(() => {
        producto = undefined;
      });
    }
  ...
  it('Productos menu should load Productos page', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem('producto/producto');
  ...
  describe('Producto page', () => {
    describe('create button click', () => {
      ...
      it('should load create Producto page', () => {
        ...
        cy.url().should('match', new RegExp('/producto/producto/new$'));
    ...
    describe('with existing value', () => {
      beforeEach(() => {
        cy.authenticatedRequest({
          method: 'POST',
          url: '/services/producto/api/productos',
          body: productoSample,
        }).then(({ body }) => {
          producto = body;
      ...
      it('last delete button click should delete instance of Producto', () => {
        cy.intercept('GET', '/services/producto/api/productos/*').as('dialogDeleteRequest');
